...
func _ready() -> void:
	...
	tw.tween_property(self, "_rise", -30.0, 0.6).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func _draw() -> void:
	...
	draw_string(font, Vector2(-10, _rise), text, HORIZONTAL_ALIGNMENT_CENTER, 40, 16, col)
...
static func spawn(parent: Node2D, pos: Vector2, val: int, col: Color = Color.WHITE) -> void:
	...
	dn.value = val
	dn.color = col
	dn.position = pos + Vector2(randf_range(-10, 10), -20)
	parent.add_child(dn)
